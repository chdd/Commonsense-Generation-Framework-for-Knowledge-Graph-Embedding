...
common_triples = []
rare_triples = []
with open('D:\Experiment\work\cge-hake\data_concept\FB15k-237_concept\\test.txt', 'r') as file:
    for line in file:
        head_entity, _, tail_entity = line.strip().split('\t')
        # 判断头实体和尾实体类型
...
with open('../fb15k_split/common_triples.txt', 'w') as file:
    file.writelines(common_triples)

with open('../fb15k_split/rare_triples.txt', 'w') as file:
