...
class TrainDataset(Dataset):
    def __init__(self, triples, nentity, nrelation, negative_sample_size, mode, rel_h, rel_t, conc_ents, ent_conc,
                 rel2nn):
    ...
    def __getitem__(self, idx):
        ...
        if len(e_filter) > 0:
            ns_size = min(self.negative_sample_size//2, len(e_filter))
    ...
    def concept_filter_h(self, head, relation):
        ...
        if str(relation) not in self.rel_h:
            return []
        rel_hc = self.rel_h[str(relation)]#关系对应的头实体概念
        set_hc = set(rel_hc)#给他去个重
        h = []
        ...
        if str(head) not in self.ent_conc:
            ...
            for conc in self.ent_conc[str(head)]:  # 对应的concept
                for ent in self.conc_ents[str(conc)]:  # 对应的实体
                    h.append(ent)
        # else:
        ...
        #     else:
        #         set_ent_conc = set([])
        #     set_diff = set_hc - set_ent_conc#关系对应的头实体概念，减去实体本身所具有的头实体概念
...
class TestDataset(Dataset):
    ...
    def similarity(self,set_A, set_B):
        ...
        # 计算 Jaccard 相似度
        similarity_score = ((intersection / union)-1)*0.2
    ...
    def __getitem__(self, idx):
            ...
            for rand_head in range(self.nentity):
                ...
                if (rand_head, relation, tail) not in self.triple_set:
                    if len(set_rel2h & set_head2conc) > 0:
                        # 如果 set_rel2h 和 set_head2conc 的交集不为空（长度大于0），说明当前随机选择的头部实体 rand_head 在概念上与给定关系 relation
                        # 有一些共同之处，这样的负样本是合适的，所以将 (0, rand_head) 添加到 tmp 列表中，其中 0 表示这是一个负样本。
                        tmp.append((self.similarity(set_rel2h,set_head2conc), rand_head))
                    else:
                        tmp.append((-1, head))
                else:
                    tmp.append((-1, head))
            ...
            for rand_tail in range(self.nentity):
                ...
                if (head, relation, rand_tail) not in self.triple_set:
                    if len(set_rel2t & set_tail2conc) > 0:
                        tmp.append((self.similarity(set_rel2t,set_tail2conc), rand_tail))
                    else:
                        tmp.append((-1, tail))
                else:
                    tmp.append((-1, tail))
        ...
        #            else (-1, head) for rand_head in range(self.nentity)]
        #     tmp[head] = (0, head) # 最终里面有两种类型的元素，分别是(0,rand_head)(-1,head)/其中本来就在里面的h，r，t变成了0 head
        # elif self.mode == 'tail-batch':
